...
class SummaryDatabase:
    """SQLite database for storing generated summaries."""
    ...
    def save_summary(
        ...
        with self.lock:
            ...
            try:
                ...
                # Store channel summaries as JSON
            ...
            try:
                ...
                for row in rows:
                    summaries.append({
                        ...
                        'channel_summaries': json.loads(row['channel_summaries']),
    ...
    def set_active_collection(self, channel_id: str, thread_ts: str):
        ...
        with self.lock:
            ...
            try:
                ...
                conn.commit()
                logger.info(f"Stored active collection for channel {channel_id}, thread {thread_ts}")
            finally:
                conn.close()
    ...
    def get_active_collection(self, channel_id: str) -> Optional[str]:
        ...
        with self.lock:
            ...
            try:
                cursor = conn.cursor()
                cursor.execute(
                    'SELECT thread_ts FROM active_collections WHERE channel_id = ?',
                    (channel_id,)
                )
                row = cursor.fetchone()
    ...
    def clear_active_collection(self, channel_id: str):
        ...
        with self.lock:
            ...
            try:
                cursor = conn.cursor()
                cursor.execute('DELETE FROM active_collections WHERE channel_id = ?', (channel_id,))
                conn.commit()
    ...
    def add_collected_report(self, channel_id: str, user_id: str, username: str, text: str, timestamp: str):
    ...
    def clear_collected_reports(self, channel_id: str):
        ...
        with self.lock:
            ...
            try:
                cursor = conn.cursor()
                cursor.execute('DELETE FROM collected_reports WHERE channel_id = ?', (channel_id,))
                conn.commit()
